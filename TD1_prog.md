...
En terme de *commits*, quelle différence constatez-vous entre cocher une (ou plusieurs) des cases *Initialize this repository with* et n'en cocher aucune ?
    > En cochant 1 ou les 3 cases on a un commit initial. Si aucune n'est cochée, aucun commit n'est fait.

    Pour la suite, ne cochez aucune de ces cases.
1. Localement, configurez `git` avec votre nom (`user.name`) et votre email (`user.email`);
    ```bash
    Configurer nom utilisateur:   git config --global prenom nom
    Configurer email utilisateur: git config --global "adresse.email@service.com"
    ```
1. Initialisez le dépôt `git` local pour le projet;
    ```bash
    On se place dans le répertoire pour notre projet, ensuite on effectue: git init
    ```
...
1. Quel OS et quel shell de commande utilisez-vous ?
    > L'OS que j'utilise est Ubuntu et le shell de commande est un shell UNIX/Linux . 
1. Quelle commande permet d'obtenir de l'aide ?
Donnez un exemple.
    ```bash
     La commande afin d obtenir de l aide est "man nom_programme". Ex: man gdb
    ```
1. Donnez la ou les commandes shell permettant de
    1. afficher les fichiers d'un répertoire triés par taille (taille affichée lisiblement)
        ```bash
        Pour afficher les fichiers d un repertoire triés par taille: "ls -lhs"
        ```
    1. compter le nombre de ligne d'un fichier
        ```bash
        Afin de compter le nombre de ligne d un fichier on fait: "wc -l nom_fichier"
        ```
